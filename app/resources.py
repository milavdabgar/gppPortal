...
parser.add_argument(
    "topic", type=str, help="Topic is required should be a string", required=True
)
parser.add_argument(
    "description",
    type=str,
...
parser.add_argument(
    "resource_link",
    type=str,
    help="Resource Link is required and should be a string",
    required=True,
)
...
class StudyMaterial(Resource):
    ...
    def get(self):
        if "inst" in current_user.roles:
            ...
            study_resources = StudyResource.query.filter(
                or_(
                    StudyResource.is_approved == True,
                    StudyResource.creator == current_user,
                )
            ).all()
    ...
    def post(self):
        ...
        study_resource = StudyResource(
            ...
            resource_link=args.get("resource_link"),
